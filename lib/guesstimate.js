...
window.getProbability = function (results) {
  ...
  const log = `
${(leftStrength * 100).toFixed(1).padStart(4)}% | ${left.rikishiName} winrate in last ${BASE_MATCHES_LIMIT} matches
${(rightStrength * 100).toFixed(1).padStart(4)}% | ${right.rikishiName} winrate in last ${BASE_MATCHES_LIMIT} matches
${(probabilityBase * 100).toFixed(1).padStart(4)}% | Base probability for a ${left.rikishiName} win

${(vs.probability * 100).toFixed(1).padStart(4)}% | ${left.rikishiName} probability vs ${right.rikishiName}
${(headToHeadFactor).toFixed(1).padStart(4)}  | Head-to-head factor for ${vs.matches} matches
-----------------------------------------
${(probability * 100).toFixed(1).padStart(4)}% | Probability for a ${left.rikishiName} win
  `.trim();
