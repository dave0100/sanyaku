...
getRikishiStats = async function(rikishiId, rikishiName) {
  const response = await fetch('https://www.sumo-api.com/api/rikishi/' + rikishiId + '/stats');
  const data = await response.json();
  
  console.log('getRikishiStats', rikishiId, rikishiName, data);
  
  return data;
};

getRikishiMatches = async function(rikishiId, rikishiName) {
  const response = await fetch('https://www.sumo-api.com/api/rikishi/' + rikishiId + '/matches');
  const data = await response.json();

  console.log('getRikishiMatches', rikishiId, rikishiName, data);
  
  return data.records;
};

getRikishiMatchesVsOpponent = async function(rikishiId, opponentId, rikishiName) {
  const response = await fetch('https://www.sumo-api.com/api/rikishi/' + rikishiId + '/matches/' + opponentId);
  const data = await response.json();

  console.log('getRikishiMatchesVsOpponent', rikishiId, rikishiName, opponentId, data);

  return data;
};

window.loadRikishiData = async function(rikishiId, opponentId, rikishiName) {
  const stats = await getRikishiStats(rikishiId, rikishiName);
  const matches = await getRikishiMatches(rikishiId, rikishiName);
  const matchesVsOpponent = await getRikishiMatchesVsOpponent(rikishiId, opponentId, rikishiName);
...
window.getSanyakuRikishi = function(records) {
  const sanyakuRanks = ["Yokozuna", "Ozeki", "Sekiwake", "Komusubi", "Maegashira"];
